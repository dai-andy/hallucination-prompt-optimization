...
class Evoluter:
    ...
    def init_pop(self):
        ...
        if args.initial_mode in ["para_topk", "para_bottomk", "para_randomk"]:
            ...
            print(f'para_population {para_population}')
        ...
        print(len(self.population), args.popsize)
    ...
    def evolute(self):
        ...
        for step in range(cur_budget + 1, args.budget):
            ...
            for j in range(args.popsize):
                logger.info(f"\nMutation {j+1}/{args.popsize}")
                logger.info(f"{'='*30}")

                # Parent selection logging
                ...
                # Log parent prompts and their scores
                logger.info("Parent A:")
                logger.info(f"Prompt: {cand_a}")
                logger.info(f"Score: {self.evaluated_prompts[cand_a][0]:.4f}")
                logger.info("\nParent B:")
                logger.info(f"Prompt: {cand_b}")
                logger.info(f"Score: {self.evaluated_prompts[cand_b][0]:.4f}")
                ...
                # Log child prompt and score
                logger.info("\nChild:")
                logger.info(f"Prompt: {child_prompt}")
                
                de_eval_res = evaluator.forward(child_prompt, eval_src, eval_tgt)
                de_scores = de_eval_res["scores"]
                logger.info(f"Score: {de_scores[-1]:.4f}")
            ...
            # After all mutations, log population stats
            logger.info(f"\n{'='*50}")
            logger.info(f"Step {step} Summary")
            logger.info(f"{'='*50}")
            logger.info(f"Best score: {best_score:.4f}")
            logger.info(f"Average score: {total_score/args.popsize:.4f}")
            logger.info("\nTop 3 prompts:")
            sorted_pop = sorted(new_pop, key=lambda x: self.evaluated_prompts[x][-1], reverse=True)
            for i, prompt in enumerate(sorted_pop[:3]):
                logger.info(f"\n{i+1}. Score: {self.evaluated_prompts[prompt][-1]:.4f}")
