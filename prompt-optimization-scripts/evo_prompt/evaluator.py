...
class Evaluator(object):
    def __init__(self, args) -> None:
        ...
        templates = json.load(open(template_file, "r", encoding="utf-8"))
...
class CLSEvaluator(Evaluator):
    def __init__(self, args):
        ...
        prompts_file = f"./data/cls/{args.dataset}/prompts.txt"
        with open(prompts_file, "r", encoding="utf-8") as f:
            self.prompts = [line.strip() for line in f.readlines()]
        self.current_prompt = self.prompts[0]  # Default to first prompt
        
    def set_prompt(self, prompt):
        """Update the current prompt being used"""
    ...
    def forward(
        ...
        if "gpt" in args.language_model:
            ...
            pred = llm_cls(
                ...
                client=self.client,
                type=args.llm_type,
                prompt_template=self.current_prompt,  # Pass the current prompt
                **self.llm_config,
            )
            hypos = list(
                map(
